...
Edge case:-The balloons which after bursting creates an out of bound condition then we need to treat out of bound index as a virtual balloon with number 1 .
    ...
    Now to think for time complexity, we have C(n, k) cases for k balloonsand for each case it need to scan the k balloons to compare.Still it's too high but It's better than O(n!) but worse than O(2 ^ n) or maybe O(3 ^ n).
    4th Intuition : -
...
So to answer the 6th Intuition, if we think with a penand paper we get that becuase only the firstand last balloons are the balloons where we are sure of their adjacent balloons before hand!Because
For the first we have nums[k - 1] * nums[k] * nums[k + 1] and for the last we have nums[-1] * nums[k] * nums[n] .
OKkk.Now think about n balloons if j is the last one to burst, We can see that the balloons are again getting separated into 2 sections.But this time since the balloon j is the last balloon of all to burst, the leftand right section now has well defined boundaryand do not affect each other!
...
Let's recap a bit:
    ...
    Now just try all the possible middle balloons to pop(the 3rd variable).For each balloon we choose, use it as the rightand left ballon of the next level of recursion, along with the padded 1's, and so forth.
    Base case is when there are no more balloons between the leftand right balloon indexes(left + 1 == right)
